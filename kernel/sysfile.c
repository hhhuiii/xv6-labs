...
sys_mmap(void)
{
  ...
  //以下情况直接返回-1
  //源文件不可读，vma却映射为可读；源文件不可写，vma却映射为可写且设置了将修改写回源文件
  ...
  struct proc *p = myproc();
  struct vma *v = 0;
  uint64 vaend = MMAPEND;//mmap区域起始位置
  //遍历查询未被使用的vma，并且计算当前已使用的vma的最低地址
  for(int i = 0; i < NVMA; i++) {
    struct vma *vv = &p->vmas[i];
    if(v->valid == 0) {//找到空闲vma
      if(v == 0) {
        v = &p->vmas[i];
        v->valid = 1;
      }
    }
    else if(vv->vastart < vaend) {//更新已使用vma的最低地址
      vaend = PGROUNDDOWN(vv->vastart);//确保新分配的mmap区域满足页对齐分配
    }
  }
  //没找到空闲的vma
  if(v == 0)
  ...
  //设置vma属性
  v->vastart = vaend - sz;//前面已经对sz取页面整数倍了，因此一定会对齐
  v->sz = sz;
  v->f = f;
  v->prot = prot;
  v->flags = flag;
...
sys_munmap(void)
{
  ...
  if(v == 0)
    return -1;
  if(addr > v->vastart && addr + sz < v->vastart + v->sz)//释放区域应该是完整vma的起点
    return -1;
  ...
  if(nunmap < 0)
    nunmap = 0;
  vmaunmap(p->pagetable, addr_alinged, nunmap, v);//从addr_alinged开始释放
